 ...
 *
 */
public class MixedThreadPool implements ThreadPool {
	private static Logger logger = LoggerFactory.getLogger(MixedThreadPool.class);
    public Executor getExecutor(URL url) {
    	
    	//线程的name的前缀
        String threadName = Constants.THREADPOOL_NAME_MIXED;
        
        //线程池core_Pool_Size值
        int corePoolSize=url.getParameter(Constants.THREADS_COREPOOLSIZE_KEY, Constants.THREADS_COREPOOLSIZE_MIXED); 
        
        //线程池maximumPoolSize值
        int maximumPoolSize = url.getParameter(Constants.THREADS_MAXIMUMPOOLSIZE_KEY, Constants.THREADS_MAXIMUMPOOLSIZE_MIXED);
        
        //线程池队列大小值 
        int queues = url.getParameter(Constants.QUEUES_KEY, Constants.DEFAULT_QUEUES_MIXED);
 
        
        //线程空闲时间超过keepAliveTime，线程将被终止
        int keepalive = url.getParameter(Constants.THREAD_ALIVE_KEY, Constants.THREAD_ALIVE_MIXED);
        
        logger.info("线程池类型="+Constants.THREADPOOL_TYPE_MIXED+",corePoolSize="+corePoolSize+",maximumPoolSize="+maximumPoolSize+",queueSize="+queues+",keepAliveTime(ms)="+keepalive);
