...
public class FixedThreadPool implements ThreadPool {
    ...
    public Executor getExecutor(URL url) {
    	...
    	//线程的name的前缀
        String threadName = Constants.THREADPOOL_NAME_FIXED;
        
        //线程池core_Pool_Size值
        int corePoolSize=url.getParameter(Constants.THREADS_COREPOOLSIZE_KEY, Constants.THREADS_COREPOOLSIZE_FIXED); 
        
        //线程池maximumPoolSize值
        int maximumPoolSize = url.getParameter(Constants.THREADS_MAXIMUMPOOLSIZE_KEY, Constants.THREADS_MAXIMUMPOOLSIZE_FIXED);
        
        //线程池队列大小值 
        int queues = url.getParameter(Constants.QUEUES_KEY, Constants.DEFAULT_QUEUES_FIXED);
 
        
        //线程空闲时间超过keepAliveTime，线程将被终止
        int keepalive = url.getParameter(Constants.THREAD_ALIVE_KEY, Constants.THREAD_ALIVE_FIXED);
        
        logger.info("线程池类型="+Constants.THREADPOOL_TYPE_FIXED+",corePoolSize="+corePoolSize+",maximumPoolSize="+maximumPoolSize+",queueSize="+queues+",keepAliveTime(ms)="+keepalive);
